...
Hrs3300::PPGState Hrs3300::AutoGain(uint16_t hrs, uint16_t als) {
  ...
  hrsHighCount = hrs > hrsHighThresh ? std::max(hrsHighCount, static_cast<uint8_t>(hrsHighCount + 1)) : 0;
  hrsLowCount = hrs < hrsLowThresh ? std::max(hrsLowCount, static_cast<uint8_t>(hrsLowCount + 1)) : 0;
  alsHighCount = als > alsHighThresh ? std::max(alsHighCount, static_cast<uint8_t>(alsHighCount + 1)) : 0;
  alsLowCount = als < alsLowThresh ? std::max(alsLowCount, static_cast<uint16_t>(alsLowCount + 1)) : 0;
